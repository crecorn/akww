...
export const RestorationCTA = EmergencyCTA

export function EmergencyCTA({ variant = 'section', service }: EmergencyCTAProps) {
  ...
  if (variant === 'emergency') {
    return (
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button
          onClick={() => openBookingModal(service || 'Water Emergency')}
          className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[var(--color-accent)] hover:bg-[var(--color-accent-dark)] text-white font-bold rounded-lg transition-colors"
        >
          Get Help Now
        </button>
