...
import { waterproofingServicesData, sectionLinksData } from '@/lib/constants/waterproofing'
...
const waterproofingServices = waterproofingServicesData.map(service => ({
  ...service,
  icon: iconMap[service.iconName] || Droplets,
}))

const sectionLinks = sectionLinksData.map(link => ({
  ...link,
...
export function WaterproofingSectionNav({ variant = 'sidebar' }: SectionNavProps) {
    ...
    <aside className="w-72 flex-shrink-0">
      <div className="sticky top-24 bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <div className="bg-[var(--color-primary)] p-4">
          <Link href="/waterproofing" className="flex items-center gap-3 text-white">
            <Droplets className="w-6 h-6" />
            <span className="font-bold text-lg">Waterproofing</span>
          </Link>
        </div>
        ...
        <div className="p-4 bg-slate-50">
          <p className="text-sm text-slate-600 mb-3">
            Have questions about basement waterproofing?
          </p>
          <Link
