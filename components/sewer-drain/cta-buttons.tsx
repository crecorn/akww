...
interface SewerDrainCTAProps {
  variant?: 'hero' | 'section' | 'emergency'
  service?: string
}

export function SewerDrainCTA({ variant = 'section', service }: SewerDrainCTAProps) {
  ...
  if (variant === 'hero') {
    return (
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button
          onClick={() => openBookingModal(service || 'Sewer & Drain')}
  ...
  if (variant === 'emergency') {
    return (
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        ...
        <button
          onClick={() => openBookingModal(service || 'Emergency Drain Service')}
          className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[var(--color-accent)] hover:bg-[var(--color-accent-dark)] text-white font-bold rounded-lg transition-colors"
        >
          Book Now
        </button>
